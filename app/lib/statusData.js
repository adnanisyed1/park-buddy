...
export async function getByways() {
  try {
    ...
    const r = await fetch(origin() + "/byways-data.js?v=10", { next: { revalidate: 86400 } });
    if (!r.ok) return [];
    const text = await r.text();
...
export async function getBywayDetail(id) {
  if (!id) return null;
  try {
    const r = await fetch(origin() + "/byways/detail/" + encodeURIComponent(id) + ".json?v=18", { next: { revalidate: 86400 } });
    if (!r.ok) return null;
    return await r.json();
  } catch {
    return null;
