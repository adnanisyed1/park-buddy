...
export default function TrailRouteChart({ trailKey, path, category }) {
  ...
  useEffect(() => {
    ...
    milestones.forEach((ms, i) => {
      ...
      const mk = new window.google.maps.Marker({
        ...
        label: { text: String(i + 1), color: "var(--pb-bg)", fontSize: "11px", fontWeight: "800" },
  ...
  if (profile === undefined) {
    return <div style={{ background: "var(--pb-surface)", border: "1px solid rgba(217,183,121,.16)", borderRadius: 24, padding: 24, textAlign: "center", color: "var(--pb-muted)", fontSize: ".84rem", marginBottom: 22 }}>Loading route &amp; elevation…</div>;
  }
  ...
  return (
    <div style={{ background: "var(--pb-surface)", border: "1px solid rgba(217,183,121,.16)", borderRadius: 24, overflow: "hidden", marginBottom: 22 }}>
      {/* Live navigation bar */}
      <div style={{ padding: "18px 20px", borderBottom: "1px solid rgba(217,183,121,.16)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div>
          {isOffline && (
            <div style={{ background: "var(--pb-surface-2)", border: "1px solid rgba(217,183,121,.22)", color: "var(--pb-gold-soft)", fontSize: ".76rem", fontWeight: 600, borderRadius: 10, padding: "7px 11px", marginBottom: 8, maxWidth: 360 }}>
              You&apos;re offline — trail imagery isn&apos;t available, but live navigation still works.
            </div>
          )}
          <div style={{ fontFamily: mono, fontSize: ".62rem", fontWeight: 800, letterSpacing: ".16em", textTransform: "uppercase", color: "var(--pb-muted)", marginBottom: 4 }}>Live navigation</div>
          {!navWatching && !navError && <div style={{ fontSize: ".8rem", color: "var(--pb-muted)" }}>Get a compass direction to stay on this trail while you hike.</div>}
          {navError && <div style={{ fontSize: ".8rem", color: "var(--pb-hold)" }}>{navError}</div>}
          {navWatching && nav && (
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                <b style={{ fontSize: ".88rem", color: "var(--pb-ink)", display: "block" }}>{nav.onTrail ? "On trail — mi " + nav.mileMark.toFixed(1) + " of " + maxMi.toFixed(1) : nav.distFt + " ft off trail"}</b>
                <span style={{ fontSize: ".76rem", color: "var(--pb-muted)" }}>{nav.onTrail ? "Keep going " + nav.compass : "Head " + nav.compass + " to rejoin the trail"}{nav.accuracyFt != null && " · ~" + nav.accuracyFt + " ft accuracy"}</span>
              </div>
            </div>
          )}
          {navWatching && !nav && <div style={{ fontSize: ".8rem", color: "var(--pb-muted)" }}>Finding your location…</div>}
        </div>
        <button onClick={navWatching ? stopNav : startNav} style={{ border: "none", borderRadius: 999, padding: "9px 16px", fontSize: ".78rem", fontWeight: 800, cursor: "pointer", fontFamily: "inherit", flex: "none", background: navWatching ? "var(--pb-ink)" : "linear-gradient(120deg,#e8cf9a,#c9a35f)", color: navWatching ? "var(--pb-bg)" : "var(--pb-bg)" }}>{navWatching ? "■ Stop navigation" : "▶ Start navigation"}</button>
      </div>

      {/* Map + Milestone photos side by side */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(340px,1fr))" }}>
        <div style={{ position: "relative", minHeight: MAP_MINH, borderRight: "1px solid rgba(217,183,121,.16)" }}>
          {mapsLoaded ? (
            <>
              <div ref={mapDivRef} style={{ position: "absolute", inset: 0 }} />
              <span style={{ position: "absolute", left: 12, top: 12, zIndex: 2, background: "rgba(21,36,28,.85)", color: "var(--pb-ink)", fontFamily: mono, fontSize: ".58rem", letterSpacing: ".14em", textTransform: "uppercase", borderRadius: 999, padding: "5px 11px" }}>Satellite · tap a number</span>
              <div style={{ position: "absolute", top: 12, right: 12, zIndex: 2, width: 34, height: 46, background: "rgba(255,253,248,.85)", borderRadius: 6, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: mono, fontSize: ".68rem", fontWeight: 700, color: "var(--pb-ink-2)", pointerEvents: "none" }}>
                <span>N</span><span style={{ fontSize: ".78rem", lineHeight: 1 }}>↑</span>
              </div>
            </>
          ) : (
            <svg viewBox={"0 0 " + MAP_W + " " + MAP_H} preserveAspectRatio="xMidYMid slice" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", display: "block", background: "var(--pb-surface-2)" }}>
              <path d={routeLine} fill="none" stroke="var(--pb-surface)" strokeWidth="11" strokeLinecap="round" />
              ...
              {msDots.map((d, i) => (
                <g key={i} onClick={() => setSelected(i)} style={{ cursor: "pointer" }}>
                  <circle cx={d.pt[0]} cy={d.pt[1]} r={i === selected ? 13 : 11} fill={i === selected ? "#e8cf9a" : "var(--pb-surface)"} stroke="var(--pb-bg)" strokeWidth="2" />
                  <text x={d.pt[0]} y={d.pt[1] + 4} textAnchor="middle" fontFamily={mono} fontSize="12" fontWeight="800" fill="var(--pb-bg)">{i + 1}</text>
                </g>
              ))}
              {routeDot && <circle cx={routeDot[0]} cy={routeDot[1]} r="7" fill={ACCENT} stroke="var(--pb-surface)" strokeWidth="2.5" />}
              {liveDot && <circle cx={liveDot[0]} cy={liveDot[1]} r="8" fill={NAV_COLOR} stroke="var(--pb-surface)" strokeWidth="2.5" />}
              <g fontFamily={mono} fontSize="13" fill="var(--pb-muted)">
                <text x="30" y="40">N</text>
                <line x1="35" y1="47" x2="35" y2="78" stroke="var(--pb-muted)" strokeWidth="1.6" />
                <path d="M35 45 l-5 9 h10 Z" fill="var(--pb-muted)" />
              </g>
              <g fontFamily={mono} fontSize="12" fill="var(--pb-muted)">
                <line x1={MAP_W - 24 - scaleBar.px} y1={MAP_H - 24} x2={MAP_W - 24} y2={MAP_H - 24} stroke="var(--pb-muted)" strokeWidth="1.6" />
        ...
        <div ref={panelRef} style={{ position: "relative", minHeight: MAP_MINH, display: "flex", flexDirection: "column", padding: "18px 18px 16px" }}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10 }}>
            <h2 style={{ fontFamily: serif, fontWeight: 700, fontSize: "1.1rem", margin: 0, color: "var(--pb-ink)" }}>Milestone photos</h2>
            <span style={{ fontFamily: mono, fontSize: ".6rem", letterSpacing: ".14em", textTransform: "uppercase", color: "var(--pb-muted)" }}>Tap a numbered marker</span>
          </div>
          {sel && (
            <div style={{ flex: 1, display: "flex", flexDirection: "column", marginTop: 10 }}>
              ...
              <div style={{ fontFamily: serif, fontWeight: 700, fontSize: "1.15rem", marginTop: 4, color: "var(--pb-ink)" }}>{sel.name}</div>
              <div style={{ fontSize: ".8rem", color: "var(--pb-muted)", marginTop: 3, lineHeight: 1.5 }}>{sel.note}</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, marginTop: 12 }}>
                {selPhotos.slice().reverse().map((p, i) => (
                  <figure key={i} style={{ position: "relative", aspectRatio: "1/1", margin: 0, overflow: "hidden", borderRadius: 12, background: "var(--pb-surface-2)" }}>
                    <img src={p.url} alt="Your photo" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} />
                    <span style={{ position: "absolute", left: 7, top: 7, background: "linear-gradient(120deg,#e8cf9a,#c9a35f)", color: "var(--pb-bg)", fontFamily: mono, fontSize: ".54rem", fontWeight: 700, letterSpacing: ".08em", borderRadius: 999, padding: "2px 7px" }}>YOU</span>
                  </figure>
                ))}
                <button onClick={addPhoto} style={{ aspectRatio: "1/1", border: "1.5px dashed #c9a35f", borderRadius: 12, background: "var(--pb-surface)", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6, fontFamily: "inherit", padding: 0 }}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#c9a35f" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z" /><circle cx="12" cy="13" r="4" /></svg>
                  <span style={{ fontFamily: mono, fontSize: ".56rem", fontWeight: 700, letterSpacing: ".1em", color: "var(--pb-gold-soft)" }}>ADD YOURS</span>
                </button>
              </div>
              <div style={{ marginTop: "auto", paddingTop: 12, fontSize: ".72rem", color: "var(--pb-muted)", lineHeight: 1.5 }}>
      ...
      <div style={{ padding: "20px 22px 18px", borderTop: "1px solid rgba(217,183,121,.16)" }}>
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
          <SectionTitle>Elevation profile</SectionTitle>
          <span style={{ fontFamily: mono, fontSize: ".68rem", color: "var(--pb-ink-2)", fontWeight: 600, whiteSpace: "nowrap" }}>
        ...
        <svg ref={svgRef} viewBox={"0 0 " + W + " " + H} style={{ width: "100%", height: "auto", display: "block", cursor: "crosshair", marginTop: 6 }} onMouseMove={handleMove} onMouseLeave={() => setScrubMi(null)}>
          ...
          {scrubMi != null && (
            <>
              <line x1={X(scrubMi)} y1={T} x2={X(scrubMi)} y2={H - B} stroke="var(--pb-ink)" strokeWidth="1" strokeDasharray="3 3" />
              <circle cx={X(scrubMi)} cy={Y(scrubFt)} r="6" fill={ACCENT} stroke="var(--pb-surface)" strokeWidth="2" />
            </>
          )}
        </svg>
        <div style={{ fontSize: ".74rem", color: "var(--pb-muted)", marginTop: 8 }}>Hover the profile — the dot traces your position on the map.</div>
      </div>

      {/* Mile by mile — real milestone names + photo chips */}
      {milestones.length > 0 && (
        <div style={{ borderTop: "1px solid rgba(217,183,121,.16)", padding: "16px 22px 22px" }}>
          <SectionTitle>Mile by mile</SectionTitle>
          <div style={{ display: "flex", flexDirection: "column" }}>
            {milestones.map((m, i) => {
              const n = photosFor(m).length;
              return (
                <div key={i} style={{ display: "flex", gap: 14, alignItems: "baseline", padding: "12px 0", borderTop: i ? "1px solid rgba(217,183,121,.16)" : "none" }}>
                  <span style={{ fontFamily: mono, fontSize: ".72rem", fontWeight: 700, color: ACCENT, flex: "none", width: 52 }}>MI {m.mi.toFixed(1)}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <b style={{ fontSize: ".95rem", color: "var(--pb-ink)" }}>{m.name}</b>
                    <button onClick={() => selectMilestone(i)} style={{ marginLeft: 8, cursor: "pointer", fontFamily: mono, fontSize: ".6rem", fontWeight: 700, color: ACCENT, background: "var(--pb-surface-2)", border: "1px solid rgba(217,183,121,.22)", borderRadius: 999, padding: "3px 9px", whiteSpace: "nowrap" }}>{n > 0 ? n + " PHOTO" + (n === 1 ? "" : "S") + " →" : "ADD PHOTO →"}</button>
                    {m.note && <div style={{ fontSize: ".8rem", color: "var(--pb-muted)", marginTop: 2 }}>{m.note}</div>}
                  </div>
                  {m.ft != null && <span style={{ fontFamily: mono, fontSize: ".7rem", color: "var(--pb-muted)", flex: "none" }}>{Math.round(m.ft).toLocaleString()} FT</span>}
                </div>
              );
            })}
          </div>
      ...
      {toast && (
        <div style={{ position: "fixed", left: "50%", bottom: 26, transform: "translateX(-50%)", zIndex: 120, background: "var(--pb-bg)", color: "var(--pb-bg)", fontSize: ".84rem", fontWeight: 700, padding: "11px 18px", borderRadius: 999, boxShadow: "0 16px 40px -16px rgba(0,0,0,.5)", maxWidth: "90vw", textAlign: "center" }}>{toast}</div>
      )}
    </div>
  );
}
