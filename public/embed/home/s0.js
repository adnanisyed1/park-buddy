...
class Component extends DCLogic {
  ...
  buildStatic(){
    ...
    if(nav){
      ...
      var EXPLORE_MENU=[['🗺','The Live Map','Parks, forests & state parks — live','/explore'],['🧭','Trip Studio','Plan a national-parks road trip','/build-trip'],['🛣','Scenic Drives','Byways & road trips','/scenic-drives'],['◉','Trip Mode','Live on-trip: photos, checklist, alerts','/trip-mode'],['🚢','Cruises','Reach the parks by sea','/cruises'],['🤿','Diving the Parks','Dry Tortugas · Channel Islands','/diving',1],['🧗','Climbing the Parks','Yosemite · Zion · Joshua Tree','/climbing',1]];
      ...
      var SHOP_MENU=[['🏔','The Park Buddy Store','Exclusive Park Buddy apparel & art','/shop'],['📖','Trip Book','Your trip, printed & bound — live','/trip-book'],['🎟','Passes','America the Beautiful + park passes','/shop?cat=passes',1],['🎒','Gear & Apparel','Packs, layers, footwear','/shop?cat=gear',1],['⛺','Camp & Cook','Tents, bags, stoves','/shop?cat=camp',1],['🧭','Navigation & Safety','GPS, satellite, first-aid','/shop?cat=nav',1],['🗺','Maps & Guides','Topo maps & guidebooks','/shop?cat=maps',1],['🔭','Optics & Cameras','Binoculars & scopes','/shop?cat=optics',1]];
      ...
      if(!document.getElementById('navBurger')){
        ...
        var SHEETS={ explore:['Explore',EXPLORE_MENU], book:['Book',BOOK_MENU], shop:['Shop',SHOP_MENU] };
        [].forEach.call(bar.querySelectorAll('.pbTab'),function(t){ t.addEventListener('click',function(){ var s=SHEETS[t.getAttribute('data-sheet')]; if(s) openSheet(s[0],s[1]); }); });
      }
    }
    ...
    var cols={agentWrap:'minmax(0,1fr) minmax(0,1.02fr)',alertsGrid:'minmax(0,1fr) minmax(0,1.1fr)',listGrid:'minmax(0,1fr) minmax(0,1.1fr)',pinesGrid:'minmax(0,1fr) minmax(0,340px)'};
